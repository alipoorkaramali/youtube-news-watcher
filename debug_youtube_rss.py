...
def fetch_soundcloud_by_ytdlp(url, limit=40):
    ...
    try:
        ...
        for idx, track in enumerate(entries[:show_limit]):
            ...
            results.append({"title": title, "link": link, "published_str": pub_str, "date_known": date_known,
                            "is_recent": is_recent, "position_ok": position_ok, "persian_match": persian_match,
                            "weekday_ok": weekday_ok, "would_accept": would_accept})
            ...
            write_output(f"   Recent: {is_recent} | Persian-today: {persian_match} | Weekday-ok: {weekday_ok}")
...
def main():
    ...
    write_output(f"SoundCloud: max_age={SOUNDCLOUD_MAX_AGE_HOURS}h | playlist_end={SOUNDCLOUD_PLAYLIST_END} | weekday-check=ON | display=40")
        ...
        if platform == 'youtube':
            ...
            if results:
                ...
                if accepted:
                    ...
                    write_output("   هیچ ترکی با فیلتر تاریخ + تاریخ شمسی + روز هفته قبول نشد.")
                write_output("📋 همه عناوین + وضعیت کلیدواژه (فقط تطابق متنی — برای دانلود دستی):")
                for r in results:
                    match = any(kw.lower() in r['title'].lower() for kw in keywords)
                    write_output(f"   {'[✅ همسان]' if match else '[  ]'} {r['title']}")
            else:
